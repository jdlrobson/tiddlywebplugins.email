...
def test_post_existing():
  #setup
  setup(store)
  tid = Tiddler("i exist","jon_private")
  ...
  store.put(tid)
  #run
  email = mailer.handle_email({"to":"post@jon.tiddlyspace.com","from":"jdlrobson@gmail.com","subject":"i exist","body":"wipeout"})

  #verify  
  tid = store.get(Tiddler("i exist","jon_private"))
  assert tid.text == "wipeout"
  assert tid.fields.get('geo.lat') == None
